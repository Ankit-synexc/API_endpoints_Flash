...
from models.user_schema import User,CreateUser , UserResponse , UserUpdate
...
async def update_user(userid : PydanticObjectId,payload: UserUpdate) -> UserResponse:
    ...
    user = await update_user(user, updates)
    return _serializer(user)

async def delete_user(user_id: PydanticObjectId) -> UserResponse:
    user = await get_user_by_id(user_id)
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail="User not found")
